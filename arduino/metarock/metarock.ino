...
void ReadSOUT() {
  ...
  if (digitalRead(P_TDCDOUT) == HIGH) value3[0] |= 1 ;
  ...
  for(int iv=0; iv<5; iv++){
    ...
    for (int i = 0; i < 32; i++) {
        ...
        if (digitalRead(P_TDCDOUT) == HIGH) value3[iv] |= 1 ;
        digitalWrite(P_SCLK, LOW);
  ...
  String outString3 = String(value3[0], HEX);
  for(int iv=1; iv<5; iv++){
    outString1 = outString1 + String(",") + String(value1[iv], HEX);
    outString2 = outString2 + String(",") + String(value2[iv], HEX);
    outString3 = outString3 + String(",") + String(value3[iv], HEX);
  }
   
  Serial.println(outString1+";"+outString2+";"+outString3);
}
